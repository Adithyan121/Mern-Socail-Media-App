...
const Navbar = () => {
  ...
  // Determine logo based on current theme mode
  ...
  const handleSearch = async () => {
    try {
      const response = await axios.get(`http://localhost:3001/users/search?name=${searchQuery}`,
      {
        method: "GET",
        headers: { Authorization: `Bearer ${token}` },
      });
      // Process the response (e.g., display search results)
